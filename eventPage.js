...
chrome.runtime.onInstalled.addListener(function(details){
	var noti = {NOTI_SWITCH_KEY:NOTI_SWITCH_ON};
...
chrome.storage.onChanged.addListener(function(changes, areaName){
	...
	if(state == NOTI_SWITCH_OFF){
		chrome.browserAction.setIcon({path: '/images/disabled19.png'});
		chrome.browserAction.setTitle({title: TITLE_ENABLE}); 
		setGlobalNotiSetting();
	} else {
		chrome.browserAction.setIcon({path: '/images/icon19.png'});
		chrome.browserAction.setTitle({title: TITLE_DISABLE}); 
		clearSettings();
	}
});
...
function switchNotiState(){
	chrome.storage.local.get(NOTI_SWITCH_KEY, function(items) {
		...
		chrome.storage.local.set({NOTI_SWITCH_KEY:value});
	});
}
